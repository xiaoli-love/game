...
// Number of cubes that must be smashed before activating a feature.激活功能之前必须粉碎的立方体数量
...
// The last known position of the primary pointer in screen coordinates.主指针在屏幕坐标中的最后一个已知位置
let pointerScreen = { x: 0, y: 0 };
// Same as `pointerScreen`, but converted to scene coordinates in rAF.与“指针屏幕”相同，但在 rAF 中转换为场景坐标
let pointerScene = { x: 0, y: 0 };
// Minimum speed of pointer before "hits" are counted.计算“命中”之前指针的最小速度
const minPointerSpeed = 60;
// The hit speed affects the direction the target post-hit. This number dampens that force.命中速度会影响目标命中后的方向。这个数字抑制了这种力量
const hitDampening = 0.1;
// Backboard receives shadows and is the farthest negative Z position of entities.篮板接收阴影，是实体最远的负 Z 位置
const backboardZ = -400;
const shadowColor = '#262e36';
// How much air drag is applied to standard objects 对标准对象施加的空气阻力是多少
const airDrag = 0.022;
const gravity = 0.3;
// Spark config 火花配置
...
// Track pointer positions to show trail 跟踪指针位置以显示轨迹
const touchTrailColor = 'rgba(170,221,255,.62)';
const touchTrailThickness = 7;
const touchPointLife = 120;
const touchPoints = [];
// Size of in-game targets. This affects rendered size and hit area.游戏内目标的大小。这会影响渲染大小和点击区域
...
// Size of target fragments 目标片段的大小
...
// Globals used to accumlate all vertices/polygons in each frame 用于累积每帧中所有顶点/多边形的全局变量
const allVertices = [];
const allPolys = [];
...
const state = {
	game: {
		...
		// Run time of current game.当前游戏的运行时间
		time: 0,
		// Player score.玩家得分
		score: 0,
		// Total number of cubes smashed in game.游戏中被粉碎的立方体总数
		cubeCount: 0
	},
	menus: {
		// Set to `null` to hide all menus 设置为“null”以隐藏所有菜单
		active: MENU_MAIN
	}
};
